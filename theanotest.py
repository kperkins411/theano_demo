...
def main(N=100, hiddenlayersize=4, numb_iterations =20):
    '''
    ...
    # correct output for each point in each cluster
    outputs = np.random.random_integers(0, 1, N)
    color_outputs = outputs*16 + 2
    ...
    #
    # # Dimensions of each point
    # # X = np.vstack([5*np.random.rand(N)-5,
    # #                5*np.random.rand(N)-5])
    # X = np.vstack([np.random.uniform(low=-6, high=6, size=N),
    #                np.random.uniform(low=-6, high=6, size=N)])
    ...
    layer_sizes = [X.shape[0], hiddenlayersize, 1]
    ...
    while iteration < numb_iterations:
        ...
        accuracy = np.mean((current_output > .5) == outputs)
        # Plot network output after this iteration

        #come up with colors
        colors =[]
        for i in range(N):
            if (current_output[0,i]>=.5 and outputs[i]== 1) or (current_output[0,i]<.5 and outputs[i]== 0):
                colors.append('green')
            else:
                colors.append('red')
